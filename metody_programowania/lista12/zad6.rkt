...
(define-type Exp
  ...
  ;;;
  (letmutrecE [x1 : Symbol] [e1 : Exp] [x2 : Symbol] [e2 : Exp] [e : Exp]))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{letmutrec {SYMBOL ANY} {SYMBOL ANY} ANY} s)
     (letmutrecE (s-exp->symbol (first (s-exp->list
                                        (second (s-exp->list s)))))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(letmutrecE x1 e1 x2 e2 e)
     (let* ([new-env (extend-env-undef (extend-env-undef env x1) x2)] ;; rozszerzmy srodowisko o x1 i x2
            [v1 (eval e1 new-env)]  ;; obliczamy e1 w nowym srodowisku
            [v2 (eval e2 new-env)]) ;; obliczamy e2 w nowym srodowisku
       (begin (update-env! new-env x1 v1)
...
(run` {let true {= 0 0}
  ...
  ;-- {letmutrec { x1 e1 } { x2 e2 } { x3 e3 } { x4 e4 } e }

#;( Aby uogolnic na dowolna liczbe wiazan to trzeba wprowadzic zmiany takie jak:
      - w skladni:
   (letrecE [bindings : (List (List Symbol Exp))] [body : Exp])

      -w parserze:
(letrecE (map (lambda (binding)
                     (list (s-exp->symbol (first binding))
